...
#### 쾨니그 정리
그래프에서 최소한의 노드로 모든 노드를 커버할 수 있는 집합을 최소 노드 커버라고 한다.  
보통, 이 최소 노드 커버를 구하기 위해서는 NP-하드 문제이기 때문에 오랜시간이 걸리는데, 이분 그래프에 한해서 쾨니그 정리를 통해서 최소 노드 커버의 크기와 최대 매칭의 크기가 항상 같음을 알 수 있다.  
이와 반대로 최소 노드 커버의 여집합 개념으로 최대 독립 집하이 존재한다. 이 또한 그래프에서는 NP-하드 문제이지만 이분 그래프에 한하여 쾨니그 정리를 통해 쉽게 구할 수 있다.  
> 그래서 쾨니그 정리가 뭔데;  

### 12.3.4 경로 커버
경로 커버는 경로 집합의 일종으로 모든 노드가 최소 하나의 경로에 속하는 경우를 말한다.  
DAG(사이클을 이루지 않는 방향 그래프)는 최소 경로 커버를 찾기 위해 최대 유량을 사용할 수 있다.  

> 이 파트 이해하기 힘들다.  

## 12.4 깊이 우선 탐색 트리
그래프에 대한 깊이 우선 탐색을 진행하면 루트가 있는 방향성 신장 트리가 만들어진다. 이를 **깊이 우선 탐색 트리** 라고 한다.  
특이한 간선이 하나 존재하는데, 이 간선을 역방향 간선이라 부르고 탐색 시작 노드에서 이웃한 노드가 깊이 우선 탐색으로 인해 탐색 당하면 탐색 시작 노드로부터 역방향 간선이 생긴다.  

### 12.4.1 이중 연결성
연결 그래프의 임의의 노드 하나를 삭제하더라고 연결 그래프 성질을 유지하면 이중 연결 그래프라고 한다.  
노드 하나를 삭제 했을 경우, 두 개 이상의 컴포넌트가 생기면 이는 이중 연결 그래프가 아니다.  
이 노드를 단절점이라 하며 비슷한 개념으로 간선을 삭제 했을 때 두 개 이상의 컴포넌트가 생기면 이를 단절선이라 부른다.  

깊이 우선 탐색 트리에서 단절점과 단절선을 쉽게 구할 수 있는데 방법은 아래와 같다.  
단절점은 노드 x가 root이며 자식 노드가 2개 이상이면 단절점이며, 노드 x가 root가 아니며 노드x의 서브트리에서 노드x의 조상으로 가는 역방향 간선이 없을 경우 단절점이라 부른다.  
또한 단절선도 비슷한 개념으로 a->b가 단절선인 경우는 트리의 간선이면서 노드 b의 서브트리에서 노드 a 혹은 a의 조상으로 가는 역방향 간선이 없어야한다.  

### 12.4.2 오일러 서브 그래프
**오일러 서브 그래프(Eulerian subgraph)**는 그래프의 모든 노드를 포함하고 있으며 간선 중 일부를 포함하면서 모든 노드의 차수가 짝수가 되는 서브그래프를 의미한다.  
공식으로 오일러 서브 그래프가 몇개가 되는지 알 수 있는데 k가 그래프에 대해 깊이 우선 탐색 트리를 그렸을 때 생기는 역방향 간선의 개수라고 한다면 오일러 서브 그래프는 항상 `2^k`개 존재한다.  
이때 `k=m-(n-1)`이 성립한다. (n은 노드 개수, m은 간선의 개수)  

> 그 뒤에서 설명을 하는데 말로 설명하니 모르겠다. 사진으로 보여줘ㅠㅠ  