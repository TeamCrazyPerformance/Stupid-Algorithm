...
내용은 모든 순열을 찾아서 풀여야할 문제를  
동적 계획법을 이용해 모든 부분집합으로 바꿔서 풀 수 있다는 것 같다.  

다만, 이 책의 예제 코드가 뭔 내용인지 이해가 안된다.  

### 6.2.5 타일 세기
타일세기 문제.
```
n*m 크기의 격자를 1*2타일과 2*1타일로 채우는 방법을 count하자.
```
책에서 소개하는 내용은 일반 문제와 달리 n*m 크기의 격자에 해당된다.   
음.. 읽으면서 이해가 안된점은 외 구지 4개의 글자로 나눠서 체크하는지 의문이다.  

각 행의 길이가 m라고 하면 행마다 체크하는 경우의 수가 `4^m`이고  
n개의 행이 있으니 `O(n4^m)`이라는 어마어마한 시간복잡도가 나오는데 말이다.  

이와 비슷한 문제로 **백준 1648**이 존재하는데
비트마스킹 기법과 동적 계획법을 병행해서 푸는 문제이다.  
차라리 더 공부해서 위 방법대로 풀어보는 것이 좋을 것 같다.  
